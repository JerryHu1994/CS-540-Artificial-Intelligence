...
public class Node{
	...
	public void calculateOutput()
	{
		...
		{
			double currOutput = 0;
			// sum up the input from its parents
			for(NodeWeightPair pair : this.parents){
				currOutput += pair.node.getOutput() * pair.weight;
			}
			this.sum = currOutput;
			//use the sigmoid function to calculate the output value
			
			
			this.outputValue = 1/(1+Math.exp(currOutput*-1));
